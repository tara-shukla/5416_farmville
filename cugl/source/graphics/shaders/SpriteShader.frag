...
// The scissor+gradient uniform block
...
vec4 blursample(vec2 coord) {
    ...
    for(int ii = 0; ii < 5; ii++) {
        // Problems with Samsung prevent a vec4 accumulator
        float r = 0.0;
        float g = 0.0;
        float b = 0.0;
        float a = 0.0;
        for(int jj = 0; jj < 5; jj++) {
            ...
            vec4 color = texture(uTexture, coord + offs);
            r += color.x*factor[jj];
            g += color.y*factor[jj];
            b += color.z*factor[jj];
            a += color.w*factor[jj];
        }
        result.x += r*factor[ii];
        result.y += g*factor[ii];
        result.z += b*factor[ii];
        result.w += a*factor[ii];
    }
